...
class EncoderDecoder(Chain):
    ...
    '''
        ...
        It connect the last encoder layer with the first decoder layer by feeding the s_state and c_state from
        two encoder (left-right and right-left). c_state is the reccurent data to feed to next layer.
    '''
    ...
    def encode(self, word, lstm_layer_list, train):
        self.feed_lstm(word, self.embed_enc, lstm_layer_list, train, dropout=DROPOUT)

    # Function to decode a target sentence word
    def decode(self, word, train):
        self.feed_lstm(word, self.embed_dec, self.lstm_dec, train, dropout=DROPOUT)
    ...
    def encode_decode_train(self, in_word_list, out_word_list, train=True, sample=False):
        ...
        for next_word_var in var_dec[1:]:
            ...
            this value simply mean how difference our predicted word with our target word, so we want to have a smaller loss.
